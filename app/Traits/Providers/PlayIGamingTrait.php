...
trait PlayIGamingTrait
{
     ...
     * @var string
     */
    public static $apiUriPIG = 'https://voxelgator.com/api/v1/';
                ...
                if($json['status']) {
                    foreach($json['providers'] as $provider) {
                        ...
                        $data = [
                            ...
                            'rtp'           => 80,
                            'status'        => 1,
                            'distribution'  => 'voxelgator',
                        ];

                        $providerCheck = Provider::where('code', $provider['code'])->where('distribution', 'voxelgator')->first();
            ...
            if($response->successful()) {
                ...
                if ($json['status'] == "success") {
                    foreach($json['games'] as $game) {
                        ...
                        $provider = Provider::where('distribution', 'voxelgator')->where('name', $game['provider_game'])->first();
                        if(!empty($provider)) {
    ...
    {
        $checkGame = Game::where('provider_id', $provider->id)->where('game_code', $game['game_id'])->where('distribution', 'voxelgator')->first();
        if(empty($checkGame)) {
    ...
    public function WebhookPIG($request)
    {
        ...
        if (!empty($gateway)) {
            if (!($gateway->getAttributes()['pig_agent_code'] == $request->agent_code && $gateway->getAttributes()['pig_agent_token'] == $request->agent_token)) {
            ...
            if(!empty($user) && !empty($wallet)) {
                ...
                if($request->type == 'win') {
                    $checkTransaction = Order::where('type', 'bet')
                        ...
                        ->where('transaction_id', $request->transaction_id_old)->first();
    ...
    private function SetRefundPIG($request)
    {
        if(isset($request->user_id)) {
            ...
            if(!empty($user) && !empty($wallet)) {
                ...
                if(!empty($checkTransaction)) {
                    $transactionCreated = self::CreatePIGTransactions($request->user_id, $request->session_id, $request->transaction_id, 'refund', 'balance', 0, $request->game);
                    if(!empty($transactionCreated)) {
                        $wallet->increment($checkTransaction->type_money, $bet);
    ...
    private static function uploadFromUrlPIG($url, $name = null)
    {
        try {
            ...
            if ($response->getStatusCode() === 200) {
                ...
                $fileName = 'venix/'.$fileName . '.' . $extension;
