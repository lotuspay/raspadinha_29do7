 ...
 public function SetTransactionWinDrakon($request)
{
    if (isset($request->user_id)) {
        ...
        if (!empty($user) && !empty($wallet)) {
            ...
            $checkTransaction = Order::where('type', 'bet')
                ->where('transaction_id', $request->transaction_id_old)
                ->where('hash', $hashGenerate)
                ->first();
     ...
     * @return false
     */
    private static function CreateDrakonTransactions($playerId, $betReferenceNum, $transactionID, $type, $changeBonus, $amount, $game, $hash)
    {
        $order = Order::create([
            'user_id'       => $playerId,
