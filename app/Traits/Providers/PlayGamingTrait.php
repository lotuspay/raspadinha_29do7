...
{
    protected static $ENDPOINT = 'https://tbs2api.dark-a.com/API/';
    ...
    public static function WebhooksPlayGaming($request)
    {
        if (self::GetCredentialsPlayGaming()) {
            ...
            if(isset($req['key'])){
                if($req['key'] != self::$PGkey){
                    return response()->json([
                        'status' => 0,
                        'error' => 'NO_BALANCE'
                    ], 400);
                }
            }elseif(isset($req['hall'])){
                if($req['hall'] != self::$hall){
                    return response()->json([
                        'status' => 0,
                        'error' => 'NO_BALANCE'
                    ], 400);
                ...
                if ($json['status'] == "success") {
                    ...
                    foreach ($gameList as $game) {
                        if (!empty($game['img'])) {
                            ...
                            $checkProvider = Provider::where('code', $providerLabel)->where('distribution', 'tbs')->first();
                            ...
                            $checkGame = Game::where('game_id', $game['id'])->where('distribution', 'tbs')->first();
                            if (empty($checkGame)) {
    ...
    private static function uploadFromUrlPlayGaming($url, $name = null)
    {
        try {
            ...
            if ($response->getStatusCode() === 200) {
                ...
                $fileName = 'tbs/' . ($name ?? $pathInfo['filename']) . '.' . ($pathInfo['extension'] ?? 'png');
