            ...
            if(!empty($pdo)) {
                ...
                if ($checkTransaction == 0 && ($request->action == 'balance' || !empty($nameGame))) {
                    if ($action == 'balance') {
                        ...
                        // NUNCA incrementa balance, sempre vai para balance_bonus quando há ganho
                        $sql = [
...
//                            ]);
    ...
    private function generateWalletChange($pdo, $request, $user, $nameGame, $historyBet = null): void
    {
        ...
        // AQUI VERIFICA SE TEM AFILIADO, ATRIBUI PORCETAGEM PARA referRewards, NGR
        // Só contabiliza RevShare da carteira balance (Saldo Depósito)
        if (!empty($user->inviter) && $title === 'loss' && $request->type_money === 'balance') {
            ...
            if (!empty($afiliadoWallet)) {
                // Buscar histórico de RevShare
                $affHistoryRevshare = $pdo->table('affiliate_histories')
                    ->where('user_id', $user->id)
                ...
                if (!empty($affHistoryRevshare)) {
                    ...
                    // Atualizar carteira do afiliado
                    $pdo->table('wallets')
                        ->where('user_id', $user->inviter)
                        ->update([
                            'refer_rewards' => $afiliadoWallet->refer_rewards + $calcReward
                        ]);

                    // Atualizar histórico de RevShare
                    $pdo->table('affiliate_histories')
                        ->where('user_id', $user->id)
                        ->where('commission_type', 'revshare')
                        ->update([
                            'commission' => $affHistoryRevshare->commission + $calcReward,
                            'commission_paid' => $affHistoryRevshare->commission_paid + $calcReward,
                            'losses' => $affHistoryRevshare->losses + 1,
                            'losses_amount' => $affHistoryRevshare->losses_amount + $request->amount
                        ]);
                }
            }
        }
