...
use App\Filament\Admin\Resources\AffiliateWithdrawResource\Pages;
...
class AffiliateWithdrawResource extends Resource
{
    ...
    {
        // Se for afiliado, mostra apenas os próprios saques pendentes
        if(auth()->user()->hasRole('afiliado')) {
            return static::getModel()::where('user_id', auth()->id())->where('status', 0)->count();
        }
        
        // Se for admin, mostra todos os saques de afiliados pendentes
    ...
    {
        $count = auth()->user()->hasRole('afiliado') 
            ? static::getModel()::where('user_id', auth()->id())->where('status', 0)->count()
            : static::getModel()::where('status', 0)->count();
